...
We run the model in R [@citeR] using the `rstanarm` package of @rstanarm and `Metrics` package of @citeMetrics. `rstanarm`@rstanarm is used to set up the model, and `Metrics` @citeMetrics is used to calculate RMSE.
...
2. **Predictive Accuracy (RMSE)**

The Root Mean Square (RMSE) typically measures the deviation between predicted values and observed values in the same metric. In the case of a logistic regression model predicting probabilities ($p_i$), the observed outcomes ($y_i$) are binary, which means they aren’t directly comparable in the usual sense of residuals as continuous values. However, RMSE can still be used as a measure of predictive performance for logistic regression by interpreting as the observed realization of the probabilities.

Logistic regression models the probability of an event occurring, $\hat{p_i}=P(y_i=1)$ while The observed binary outcome (y_i) is an approximation of whether the event occurred (1) or not (0). The squared error between the predicted probability and the binary outcome measures how well the model assigns probabilities close to the actual outcome. Thus, the equation of the Squared Error is :

$$Squared \: Error = (y_i-\hat{p_{i}})^2$$

RMSE aggregates this squared error across all observations and provides a measure of how well the model predicts probabilities in line with the observed binary outcomes. Consequently, the RMSE equation is:

$$RMSE = \sqrt{\frac{1}{n}\sum_{i=1}^{n}(y_{i}-\hat{p_{i}})^2}$$
...
A low RMSE indicates that the predicted probabilities are close to the observed outcomes, demonstrating good predictive accuracy.
